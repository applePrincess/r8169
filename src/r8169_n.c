                              ...
                              u32 opts2)
{
        ...
        for (cur_frag = 0; cur_frag < nr_frags; cur_frag++) {
                ...
                status = rtl8169_get_txd_opts1(opts1, len, entry);;
...
static int
rtl8169_sw_padding_short_pkt(struct rtl8169_private *tp,
                             struct sk_buff *skb,
                             u32 opts1,
                             u32 opts2)
{
...
static int rtl8169_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
        ...
        if (dev->features & NETIF_F_TSO) {
                ...
                if (mss) {
                        opts1 |= LargeSend | (min(mss, MSS_MAX) << MSSShift);
                        large_send = 1;
                }
        }
#endif //LINUX_VERSION_CODE > KERNEL_VERSION(2,6,0)
        ...
        if (frags) {
                ...
                if (tp->UseSwPaddingShortPkt && len < 60) {
                        if (unlikely(rtl8169_sw_padding_short_pkt(tp, skb, opts1, opts2)))
                                goto err_dma_1;
                        opts1 |= FirstFrag;
                        frags++;
                } else {
